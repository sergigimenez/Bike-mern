...
const getCardByHash = async (req, res = response) => {
  const { id } = req.params;

  const card = await Card.find({ nameURL: id });
...
const addCard = async (req, res = response) => {
  ...
  try {
    ...
    if (typeof error?.errors != "undefined") {
      ...
      if (
        (await (
          await Card.find({ titleCard: card.titleCard }).exec()
        ).length) != 0
      ) {
...
const followCardByUser = async (req, res = response) => {
  ...
  try {
    ...
    let cardExist = await (
      await Card.find({ id: { $in: cards } })
    ).map((e) => {
      return { id: e.id, title: e.titleCard, date: e.info.Fecha };
    });

    if (
      !!!usuario.cards.find((uc) => {
        return uc.id == cardExist[0]?.id;
      })
    ) {
  ...
  try {
    ...
    if (params.distancia != null) {
      if (params.distancia.lte != null) {
        query["info.Distancia"] = {
          $elemMatch: { $lte: params.distancia.lte },
        };
      }
      if (params.distancia.gte != null) {
        query["info.Distancia"] = {
          $elemMatch: { $gte: params.distancia.gte },
        };
      }
      if (params.distancia.gte != null && params.distancia.lte != null) {
        query["info.Distancia"] = {
          $elemMatch: {
            $gte: params.distancia.gte,
            $lte: params.distancia.lte,
          },
        };
      }
    }
    ...
    if (params.desnivel != null) {
      ...
      if (params.desnivel.gte != null && params.desnivel.lte != null) {
        query["info.Desnivel"] = {
          $elemMatch: { $gte: params.desnivel.gte, $lte: params.desnivel.lte },
        };
      }
    }
...
const cardUpdateNameURL = async (req, res = response) => {
  ...
  try {
    ...
    cards.forEach(async function (card) {
      let namePage = card.titleCard
        .replaceAll(/\b(?:á|à|è|é|ó|ò|ñ|\s)\b/gi, "")
        .toLocaleLowerCase();

      const nameURLUpdated = await Card.findOneAndUpdate(
        { id: card.id },
        {
          nameURL: namePage,
        },
    ...
    logError(req, res, e, "updateNameURL");
  }
};
